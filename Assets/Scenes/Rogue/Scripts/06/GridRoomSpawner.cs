...
public class GridRoomSpawner : MonoBehaviour
{
    ...
    // Probability for room is a dark room
    [Range(0, 100)]
    public int probabilityOfDarkRoom = 10;


    // Spawn Inspector Settings
    ...
    private void AddRoomToMap(RoomData roomData, bool isSpawnRoom = false)
    {
        ...
        if (roomComponent != null)
        {
            ...
            // Determine if room is a dark room based on probability
            if (!isSpawnRoom && UnityEngine.Random.Range(0, 100) < probabilityOfDarkRoom)
            {
                roomComponent.isDarkRoom = true;
            }
        }
    }
